...
//    }
    ...
    public ChessPiece performChessMove(ChessPosition sourcePosition, ChessPosition targetPosition) {
        ...
        if (testCheck(currentPlayer)) {
            ...
            throw new ChessException("You can't put yuorself in check");
        }
    ...
    private Piece makeMove(Position source, Position target) {
        ...
        p.increaseMoveCount();
        Piece capturedPice = board.piece(target);
        board.placePiece(p, target);

        if (capturedPice != null) {
            piecesOnTheBoard.remove(capturedPice);
            capturedPieces.add(capturedPice);
        }
        ...
        if (p instanceof Pawn) {
            if (source.getColumn() != target.getColumn() && capturedPice == null) {
                ...
                capturedPice = board.removePiece(pawnPosition);
                capturedPieces.add(capturedPice);
                piecesOnTheBoard.remove(capturedPice);
            }
        }
    ...
    private void  validateSourcePosition(Position position) {
        ...
        if (currentPlayer != ((ChessPiece) board.piece(position)).getColor())
            throw new ChessException("There chosen piece not is yuors");

        if (!board.piece(position).isThereAnyPossibleMove())
            throw new ChessException("There is not a possible moves");
    }
